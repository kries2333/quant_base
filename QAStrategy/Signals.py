...
# 修改简单布林
def signal_kries(df, para=[200, 2, 0.05]):
    """
        针对原始布林策略进行修改。
        bias = close / 均线 - 1
    ...
    df.drop(['raw_signal', 'std', 'bias', 'temp', 'signal_long', 'signal_short'], axis=1,
            inplace=True)

    return df

def signal_kries_para_list(m_list=range(20, 1000+20, 20), n_list=[i / 10 for i in list(np.arange(3, 50+2, 2))],
