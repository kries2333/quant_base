...
def signal_std1_leverage_bolling(df, para=[200, 2, 3]):
    """
    :param df:
    :param para: n, m
    :return:
    ...
    # 计算收盘价的标准差及其均值
    df['std'] = df['close'].rolling(n, min_periods=1).std(ddof=0)  # ddof代表标准差自由度
    df['std_ma'] = df['std'].rolling(n, min_periods=1).mean()
    df['std_ma2'] = df['std'].rolling(s*n, min_periods=1).mean()
    # 计算std_ma偏离std_ma2的值r，作为构建布林宽度的自适应值
    df['r'] = df['std_ma'] / df['std_ma2']
    # 构建布林上下轨，m*r达到了布林宽度随波动率自适应的效果
    df['upper'] = df['median'] + m * df['r'] * df['std']
    df['lower'] = df['median'] - m * df['r'] * df['std']
    ...
    df['signal'] = temp['signal']
    # 把信号融入杠杆信息
    df['signal1'] = df['signal'] * leverage / df['r']

    # ===删除无关变量
    # df.drop(['median', 'std', 'upper', 'lower', 'signal_long', 'signal_short'], axis=1, inplace=True)
    df.drop(['signal_long', 'signal_short',
             #'signal1'
             ], axis=1, inplace=True)
...
# 策略参数组合
def signal_std1_leverage_bolling_para_list(n_list=range(20, 1000+20, 20),  s_list=range(1, 12 ,1),
                                  m_list=[i / 10 for i in list(np.arange(3, 50+2, 2))]):
    """
    产生布林 策略的参数范围
    :param m_list:
    :param n_list:
