...
from app.cta_strategy.base import BacktestingMode, INTERVAL_DELTA_MAP, StopOrderStatus, EngineType, StopOrder, \
    STOPORDER_PREFIX
from app.cta_strategy.template import CtaTemplate
from trader.constant import Interval, Exchange, Status, Direction, Offset
from trader.object import TickData, BarData, TradeData, OrderData
from trader.database import database_manager
from trader.utility import round_to, parse8601
import pandas as pd
...
class BacktestingEngine:
    ...
    def load_data(self):
        ...
        #     ti = parse8601(str(h.datetime))
        #     dt = pd.to_datetime(ti, unit='ms')  # 整理时间
        #     values = [dt,
        #               h.open_price,
        #               h.high_price,
        #               h.low_price,
        ...
        #
        # _df = pd.DataFrame(df_list, columns=['candle_begin_time', 'open', 'high', 'low', 'close', 'volume'])
        # _df.to_pickle('btc_mysql_1m.pkl')
    ...
    # def put_candles(self, data):
    #     self.candle_data.append(data)

    def clear_indicators(self):
        self.indicator_data.clear()

    def put_indicators(self, data):
        # self.indicator_data.append(data, ignore_index=False)
